...
public class CommonClass extends BaseClass {
	...
	String deleteCouponMessage = "Your coupon was successfully removed.";
	String couponSuccessMessage = "You have successfully applied";
	...
	public void stripeOption() throws InterruptedException {
		...
		try {
			...
			Assert.assertTrue("5off coupon is not applied successfully",
					success_Message.contains(couponSuccessMessage));
	...
	public void removeCoupon() throws InterruptedException {
		...
		try {
			...
			Assert.assertTrue("5off coupon is not deleted successfully",
					delete_Coupon_Message.contains(deleteCouponMessage));
	...
	public void signoutApplication() {
		...
		try {
			...
			String actualUrl = "https://www.slidegeeks.com/";

			Assert.assertEquals("user is not logout from the application", expectedUrl, actualUrl);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	 ...
	 * 
	 * try { WebElement iframe = driver.findElement(By.
	 * xpath("//iframe[@id = 'chat-widget' or @name = 'chat-widget']"));
	 * Thread.sleep(1000); boolean value = iframe.isDisplayed();
	 * System.out.println("value = " + value); if (value == true) {
	 * driver.switchTo().frame(iframe); WebElement chat1 =
